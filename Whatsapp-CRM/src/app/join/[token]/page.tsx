...
export default function JoinPage() {
  ...
  const loadPeekAndAuth = useCallback(async () => {
    ...
    try {
      const [peekRes, meRes] = await Promise.all([
      ...
      setPeek(peekBody);
      // /api/me returns { profile: { id, ... } } when authenticated,
      // or 401 when signed out.
      if (meRes.ok) {
        const meBody = await meRes.json();
        setAuthedUserId(meBody.profile?.id ?? null);
      } else {
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    setSigningOut(true);
    try {
      // POST to NextAuth sign-out endpoint (csrf token not required for
      // same-origin server-actions; using the redirect=false variant so
      // we control the navigation ourselves and preserve the invite URL).
