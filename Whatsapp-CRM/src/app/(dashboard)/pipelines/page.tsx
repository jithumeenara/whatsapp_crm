...
export default function PipelinesPage() {
  ...
  const loadStages = useCallback(async (pipelineId: string): Promise<PipelineStage[]> => {
    try {
  ...
  const seedDefaultPipeline = useCallback(async (): Promise<Pipeline | null> => {
    try {
      ...
      if (!res.ok) {
        console.error("Failed to seed pipeline:", res.status);
        return null;
      }
      const body = await res.json();
      return body.pipeline as Pipeline;
    } catch (err) {
  ...
  async function handleCreatePipeline() {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? "Failed to create pipeline");
      setCreating(false);
      return;
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="start"
              className="w-64 border-border bg-card text-foreground/80"
            >
              {pipelines.length === 0 && (
