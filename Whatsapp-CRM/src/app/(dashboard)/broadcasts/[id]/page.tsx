            ...
            <div key={step.label} className="flex items-center gap-3">
              ...
              <div className="relative h-7 flex-1 rounded-full bg-muted">
                ...
                <span className="absolute inset-0 flex items-center px-3 text-xs font-medium text-foreground">
                  ...
                  <span className="ml-2 text-foreground/80/80">
                    ({pctOfSent}%)
                  </span>
                </span>
              </div>
...
export default function BroadcastDetailPage() {
  ...
  async function handleDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      toast.error(`Failed to delete: ${body?.error ?? `HTTP ${res.status}`}`);
      return;
    }
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-sm">
            ...
            <Button
              ...
              disabled={deleting}
              className="h-7 border-border bg-transparent text-foreground/80 hover:bg-muted"
            >
              Cancel
            </Button>
            <Button
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="border-border bg-card">
                <DropdownMenuItem
                  ...
                  className={
                    statusFilter === 'all' ? 'text-primary' : 'text-foreground/80'
                  }
                >
                  All statuses
                </DropdownMenuItem>
