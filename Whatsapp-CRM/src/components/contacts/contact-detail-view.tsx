...
export function ContactDetailView({
  ...
  const loadContactData = useCallback(async () => {
    ...
    const [contactRes, tagsRes, notesRes, cvRes] = await Promise.all([
      ...
      fetch(`/api/contacts/${contactId}/notes`, { cache: 'no-store' }).then((r) => r.json()),
      fetch(`/api/contacts/${contactId}/custom-values`, { cache: 'no-store' }).then((r) => r.json()),
    ]);

    const c: Contact | null = contactRes.contact ?? null;
    if (c) {
      setContact(c);
      setEditName(c.name ?? '');
    ...
    const cTags: { tag_id: string }[] = contactRes.tags ?? [];
    setContactTagIds(cTags.map((t: { tag_id?: string; id?: string }) => t.tag_id ?? t.id ?? '').filter(Boolean));

    setNotes(notesRes.notes ?? []);
    setLoadingNotes(false);

    setCustomFields(cvRes.fields ?? []);
    const map: Record<string, string> = {};
    for (const v of (cvRes.values ?? []) as { custom_field_id: string; value?: string }[]) {
  ...
  async function toggleTag(tagId: string) {
    ...
    if (isSelected) {
      const res = await fetch(`/api/contacts/${contactId}/tags?tag_id=${encodeURIComponent(tagId)}`, {
        method: 'DELETE',
      });
      if (res.ok) {
