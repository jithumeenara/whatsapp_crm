...
export function summarizeNode(node: BuilderNode): string | null {
  ...
  switch (node.node_type) {
    ...
    case "save_to_table": {
      const mappings = Array.isArray(cfg.field_mappings) ? (cfg.field_mappings as Array<Record<string, string>>) : [];
      return mappings.length > 0 ? `${mappings.length} field${mappings.length > 1 ? 's' : ''} saved` : 'No mappings yet';
    }
  }
}
