...
function SaveToTableForm({
  ...
  const [tableFields, setTableFields] = useState<TableInfo["fields"]>([]);
  ...
  // When table changes, load its fields
  useEffect(() => {
    if (!cfg.table_id) { setTableFields([]); return; }
    setLoadingFields(true);
    fetch(`/api/data-tables/${cfg.table_id}`)
      .then((r) => r.json())
      .then((d) => setTableFields(d.table?.fields ?? []))
  ...
  const updateMapping = (i: number, patch: Partial<{ field_key: string; value: string }>) => {
    onUpdateConfig({
      field_mappings: mappings.map((m, idx) => idx === i ? { ...m, ...patch } : m),
    });
  };

  const addMapping = () => {
    onUpdateConfig({ field_mappings: [...mappings, { field_key: "", value: "" }] });
  };

  const removeMapping = (i: number) => {
    onUpdateConfig({ field_mappings: mappings.filter((_, idx) => idx !== i) });
  };
        ...
        <div className="space-y-2">
          ...
          {mappings.map((m, i) => (
            <div key={i} className="flex gap-2 items-start">
              <Select
                value={m.field_key}
                onValueChange={(v) => v && updateMapping(i, { field_key: v })}
              >
                ...
                <SelectContent>
                  {tableFields
                    .filter((f) => !['section_header', 'html_block'].includes(f.field_type))
                    .map((f) => (
          ...
          >
            <Plus className="size-3" /> Add field mapping
          </button>
        </div>
      )}
...
function ConditionForm({
  ...
  // Collect all var_key values from collect_input nodes in this flow
