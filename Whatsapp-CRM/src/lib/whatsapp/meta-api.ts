...
export interface SendMediaMessageArgs {
  ...
  to: string
  kind: MediaKind
  /** Public URL Meta fetches at send time. Use this OR id — not both. */
  link?: string
...
/**
 * Upload a local file buffer to Meta's Media API and return the media_id.
 * The media_id can then be used in sendMediaMessage({ id }) — no public URL needed.
 */
export async function uploadMediaToMeta(args: {
  phoneNumberId: string
  accessToken: string
  fileBuffer: Buffer
  mimeType: string
 ...
 * Provide either `id` (from uploadMediaToMeta) or `link` (public HTTPS URL).
 */
