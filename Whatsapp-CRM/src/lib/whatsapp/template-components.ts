...
function buildButtonPayload(b: TemplateButton): MetaButtonPayload {
  switch (b.type) {
    ...
    case 'FLOW': {
      const flowAction = b.flow_action || 'navigate';
      const btn: MetaButtonPayload = {
        type: 'FLOW',
        text: b.text,
        flow_id: b.flow_id,
        flow_action: flowAction,
      };
      // navigate_screen is required for 'navigate' but must be omitted for 'data_exchange'
      if (flowAction !== 'data_exchange') {
        btn.navigate_screen = b.navigate_screen || 'DEFAULT';
      }
      return btn;
    }
  }
