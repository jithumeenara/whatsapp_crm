...
export interface SendTemplateNodeConfig {
  ...
  /** Comma-separated body variable values replacing {{1}}, {{2}} etc. */
  body_params?: string;
  next_node_key: string;
}
...
export interface SaveToTableNodeConfig {
  ...
  table_id: string;
  /** Each entry maps a DataField key to a literal value or {{vars.xxx}} template */
...
export type FlowTriggerConfig =
  ...
  | { trigger_type: "always"; config: Record<string, never> }
...
export interface FlowRow {
  ...
  trigger_type: "keyword" | "first_inbound_message" | "always" | "manual";
